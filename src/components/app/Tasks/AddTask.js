...
export default function AddTask({ isOpen, setIsOpen, id, getAll, editData }) {
  ...
  const { handleChange, handleSubmit, values, errors, touched, handleBlur, setValues } =
    useFormik({
      initialValues: { name: "", title: "", description: "", isCompleted: "0" },
  ...
  const addTask = (val) => {
    ...
    createAxiosWithToken(authenticate.loginData.token)
      .post("TaskManager/AddTasks", val)
      .then((res) => {
        if (res.data) {
          toast.open("Task Added Successfully.", "text-green-600", <MdOutlineDone />);
          setIsOpen(!isOpen);
          getAll();
          setValues({ name: "", title: "", description: "", isCompleted: "0" });
        } else {
  ...
  return (
    <Model title={id ? "Edit Task" : "New Task"} isOpen={isOpen} setIsOpen={setIsOpen}>
